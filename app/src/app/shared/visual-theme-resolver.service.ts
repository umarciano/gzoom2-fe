 ...
 */
@Injectable()
export class VisualThemeResolver implements Resolve<void | UserPreference> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<void | UserPreference> {
    ...
    const userPreferenceService$ = this.userPreferenceService.getUserPreference('VISUAL_THEME')
